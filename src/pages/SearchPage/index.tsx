      ...
      <div className="trending-strip-scroll">
        {items.map((item, i) => {
          ...
          return (
            <button 
              key={item.id} 
              className="trending-strip-card" 
              onClick={() => router.push(`/manga/${item.id}`)}
              aria-label={`Ver manga en tendencia: ${mangaProvider.getLocalizedTitle(item)}`}
            >
              <SmartImage 
                src={cover} 
                alt="" 
              ...
              <p className="trending-strip-title">{mangaProvider.getLocalizedTitle(item)}</p>
            </button>
          );
        })}
      </div>
...
const SearchPage: React.FC = () => {
  ...
  const renderDiscoveryState = () => (
    <div className="empty-state animate-fade-in">
      ...
      <h3>Descubre tu Próxima Historia</h3>
      <p>Explora miles de mangas en nuestra biblioteca global.</p>
      <div className="discovery-actions">
        <IonButton fill="outline" shape="round" onClick={() => handleSearch('Trending')}>
          <IonIcon icon={trendingUpOutline} slot="start" /> En Tendencia
        </IonButton>
        <IonButton fill="outline" shape="round" onClick={() => handleSearch('Popular')}>
          <IonIcon icon={heartOutline} slot="start" /> Populares
        </IonButton>
      </div>
    </div>
  );
