...
const ALL_ANIME_PROVIDERS = [...PROVIDERS_LIST.ANIME, MonosChinos];
...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
  ...
  (fastify.get(
    ...
    fastify.get('/random-anime', async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const res = await anilist.fetchRandomAnime().catch((err: any) => {
  ...
  fastify.get('/episodes/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const providerName = anilist.provider?.name?.toLowerCase() || 'default';
      redis
        ? reply
            .status(200)
            .send(
              await cache.fetch(
                redis,
                `anilist:episodes;${id};${dub};${fetchFiller};${providerName}`,
                async () =>
      ...
      console.error("[Anilist Episodes Error]:", err);
      return reply.status(500).send({ 
        message: err.message || 'Error fetching episodes', 
        stack: err.stack,
        details: "Check if the provider site is accessible or if the ID is valid."
      });
    }
  });
  ...
  fastify.get('/info/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const providerName = anilist.provider?.name?.toLowerCase() || 'default';
      console.log(`[Anilist Info] Fetching info for ID: ${id} with provider: ${providerName}`);
      
      let res;
      if (redis) {
          res = await cache.fetch(
            redis,
            `anilist:info;${id};${isDub};${fetchFiller};${providerName}`,
            async () => await anilist.fetchAnimeInfo(id, isDub as boolean, fetchFiller as boolean),
            dayOfWeek === 0 || dayOfWeek === 6 ? 60 * 120 : (60 * 60) / 2,
          );
      } else {
          res = await anilist.fetchAnimeInfo(id, isDub as boolean, fetchFiller as boolean);
      }
      
      if (!res) throw new Error("Anilist provider returned empty data");
      reply.status(200).send(res);
    } catch (err: any) {
      console.error("[Anilist Info Error]:", err);
      // Fallback or detailed error
      reply.status(500).send({ 
        message: err.message || 'Error fetching anime info',
        stack: err.stack,
        suggestion: "Try selecting a different provider like Gogoanime or MonosChinos."
      });
    }
  });
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const providerInstance = generateAnilistMeta(provider);

      try {
        if (!providerInstance) throw new Error("Could not initialize Anilist Meta provider");

        const fetchSources = async () => {
          return provider === 'zoro' || provider === 'animekai'
            ? await providerInstance.fetchEpisodeSources(
                episodeId,
                server,
                isDub ? SubOrSub.DUB : SubOrSub.SUB,
              )
            : await providerInstance.fetchEpisodeSources(episodeId, server);
        };

        if (redis) {
          const cacheKey = `anilist:watch;${episodeId};${providerInstance.provider.name.toLowerCase()};${server};${isDub ? 'dub' : 'sub'}`;
          const cached = await cache.fetch(redis, cacheKey, fetchSources, 600);
        ...
        console.error("[Anilist Route Error]:", err);
        reply
          .status(500)
          .send({ message: err.message || 'Unknown error', stack: err.stack });
      }
    },
  );
...
const generateAnilistMeta = (provider: string | undefined = undefined) => {
  if (typeof provider !== 'undefined') {
    const possibleProvider = ALL_ANIME_PROVIDERS.find(
